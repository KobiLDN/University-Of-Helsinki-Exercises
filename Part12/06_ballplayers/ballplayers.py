...
def most_goals(team_list: list ):
    return max(team_list, key=lambda team: team.goals).name

def most_points(team_list: list):
    player = max(team_list, key=lambda team: team.goals+team.passes)
    return (player.name, player.number)

def least_minutes(team_list: list):
    player = min(team_list, key=lambda team: team.minutes)
    return player
